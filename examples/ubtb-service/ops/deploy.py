#!/usr/bin/env python3
"""Deploy UBTB project to remote server and run the full MVP flow.

All credentials and paths are read from environment variables.
...
def _require_env(name):
    """Read a required environment variable or exit with error."""
...
SUDO_PASS = os.environ.get("DEPLOY_SUDO_PASSWORD", PASS)
REMOTE_BASE = os.environ.get("DEPLOY_REMOTE_BASE", "/home/user/ubtb-project")
LOCAL_PROJECT = os.environ.get("LOCAL_PROJECT_DIR", "")
LOCAL_TEST_ASSETS = os.environ.get("LOCAL_TEST_ASSETS_DIR", "")
...
if __name__ == "__main__":
    ...
    if action == "upload":
        if not LOCAL_PROJECT:
            print("ERROR: LOCAL_PROJECT_DIR environment variable is required")
            sys.exit(1)
        ...
        if not LOCAL_TEST_ASSETS:
            print("ERROR: LOCAL_TEST_ASSETS_DIR environment variable is required")
            sys.exit(1)
...
cat docker/docker-compose.yml | grep '8081'"""
        ...
        test_email = os.environ.get("TEST_EMAIL", "admin@ubtb.com")
...
python3 generated/ubtb-service/core-flow.py 2>&1"""
        ...
        cmd = (
            ...
            f"export TEST_PASSWORD={test_password} && "
            f"export ENVIRONMENT=test && "
            f'export APP_CMD="{app_cmd}" && '
            f"export HEALTH_URL={base_url}/actuator/health && "
