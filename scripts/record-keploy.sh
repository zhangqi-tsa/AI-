#!/usr/bin/env bash
# Keploy record runner - safely execute Keploy record and run flow script
set -euo pipefail

# ── Environment variables ──────────────────────────────────────
ENVIRONMENT="${ENVIRONMENT:-test}"
APP_CMD="${APP_CMD:-java -jar target/example-service.jar}"
HEALTH_URL="${HEALTH_URL:-http://127.0.0.1:8080/actuator/health}"
FLOW_SCRIPT="${FLOW_SCRIPT:-generated/example-service/core-flow.py}"
SERVICE_NAME="${SERVICE_NAME:-example-service}"
STARTUP_TIMEOUT_SECONDS="${STARTUP_TIMEOUT_SECONDS:-120}"
KEPLOY_DIR="${KEPLOY_DIR:-keploy}"
KEPLOY_READY_WAIT_SECONDS="${KEPLOY_READY_WAIT_SECONDS:-5}"
...
# ── Start Keploy record ────────────────────────────────────────
echo "[$(date '+%H:%M:%S')] [RECORD] Starting Keploy record for $SERVICE_NAME..."
echo "[$(date '+%H:%M:%S')] [RECORD] App command: $APP_CMD"
RECORD_START=$(date +%s)
...
if (( elapsed >= STARTUP_TIMEOUT_SECONDS )); then
    echo "[$(date '+%H:%M:%S')] ERROR: Application did not become healthy within ${STARTUP_TIMEOUT_SECONDS}s"
    exit 1
fi

# ── Wait for Keploy eBPF readiness ─────────────────────────────
echo "[$(date '+%H:%M:%S')] [READY] Waiting ${KEPLOY_READY_WAIT_SECONDS}s for Keploy eBPF probes to attach..."
sleep "$KEPLOY_READY_WAIT_SECONDS"
echo "[$(date '+%H:%M:%S')] [READY] Keploy should be ready"

# ── Execute flow script ────────────────────────────────────────
echo "[$(date '+%H:%M:%S')] [FLOW] Executing flow script: $FLOW_SCRIPT"
...
# ── Stop Keploy record ─────────────────────────────────────────
echo "[$(date '+%H:%M:%S')] [RECORD] Stopping Keploy record..."
keploy record --stop 2>/dev/null || true
wait "$KEPLOY_PID" 2>/dev/null || true
KEPLOY_PID=""
